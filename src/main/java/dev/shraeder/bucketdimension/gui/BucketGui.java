...
public final class BucketGui {
    ...
    private static ItemStack makeFluidItem(Material material, String name, StorageManager storage, Player player, FluidType type) {
        ...
        if (storage.hasUnlimitedDispense(player.getUniqueId(), type)) {
            lore.add(ChatColor.GRAY + "Max: " + ChatColor.WHITE + "Not applied while primed");
        ...
        if (type == FluidType.WATER && storage.isWaterPrimingEnabled()) {
            lore.add(ChatColor.DARK_GRAY + "Store 2 water sources to prime infinite water.");
        }
